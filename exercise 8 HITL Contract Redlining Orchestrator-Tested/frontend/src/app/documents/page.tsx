...
import { useApp, appActions, Document } from "../../contexts/AppContext";
...
export default function DocumentsPage() {
  ...
  // Use documents from global state
  ...
  const refresh = async () => {
    try {
      ...
      dispatch(appActions.setDocuments(mockDocs));
      setError(null);
    } catch (err) {
  ...
  useEffect(() => {
    // Only load mock data if no documents exist in localStorage
    if (docs.length === 0) {
      refresh();
    }
  }, [docs.length]);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      // Add mock uploaded files to the global state
      ...
      // Add each document to global state (this will also save to localStorage)
      newDocs.forEach(doc => {
        dispatch(appActions.addDocument(doc));
      });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          {docs.length > 0 && docs.every(doc => ['doc_001', 'doc_002', 'doc_003'].includes(doc.doc_id)) && (
