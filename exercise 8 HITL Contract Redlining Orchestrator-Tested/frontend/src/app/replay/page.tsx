...
export default function ReplayPage() {
  ...
  // Get completed runs from global state, fallback to mock data for new users
  const completedRuns = state.runs.filter(run => run.status === 'completed');
  const displayRuns = completedRuns.length > 0 ? completedRuns : mockRuns;
  ...
  // Auto-refresh runs every 30 seconds for real users
  useEffect(() => {
    if (!isNewUser) {
      const interval = setInterval(() => {
        // In real implementation, this would call api.getRuns() to refresh data
        // For now, we rely on global state updates
        console.log('Auto-refreshing runs...');
      }, 30000);
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      // In real implementation, this would call api.getRuns()
  ...
  const handleLoadTrace = (runId: string) => {
    ...
    if (isRealRun) {
      ...
      // For mock data, use the mock trace details
      setTraceDetails(mockTraceDetails);
    }
  ...
  const handleReplay = async () => {
    ...
    try {
      ...
      console.log('Replay Run completed for run:', selectedRun);
    } catch (error) {
  ...
  const handleCloneModify = async () => {
    if (!selectedRun) return;
    
    setCloning(true);
    try {
      // In real implementation, this would create a new run with modified parameters
      await new Promise(resolve => setTimeout(resolve, 1500)); // Simulate API call
      const newRunId = `run_clone_${Date.now()}`;
      setReplayResult(newRunId);
      setShowComparison(true);
      console.log('Clone & Modify completed for run:', selectedRun);
    } catch (error) {
      console.error('Failed to clone run:', error);
    } finally {
      setCloning(false);
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm ${isNewUser ? 'text-orange-600' : 'text-green-600'}`}>
                {isNewUser ? 'Demo Runs' : 'Completed Runs'}
              </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm ${isNewUser ? 'text-orange-600' : 'text-blue-600'}`}>
                {isNewUser ? 'Demo Total' : 'Total Runs'}
              </p>
              <p className={`text-2xl font-bold ${isNewUser ? 'text-orange-700' : 'text-blue-700'}`}>
                {isNewUser ? mockRuns.length : state.runs.length}
              </p>
              {isNewUser && (
                <p className="text-xs text-orange-600 mt-1">Mock data</p>
              )}
            </div>
            <FileText className={`w-8 h-8 ${isNewUser ? 'text-orange-600' : 'text-blue-600'}`} />
          </div>
        </div>
        <div className={`card ${isNewUser ? 'bg-orange-50 border-orange-200' : 'bg-orange-50 border-orange-200'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm ${isNewUser ? 'text-orange-600' : 'text-orange-600'}`}>
                {isNewUser ? 'Demo Progress' : 'In Progress'}
              </p>
              <p className={`text-2xl font-bold ${isNewUser ? 'text-orange-700' : 'text-orange-700'}`}>
                {isNewUser ? 0 : state.runs.filter(run => run.status === 'running').length}
              </p>
              {isNewUser && (
                <p className="text-xs text-orange-600 mt-1">Mock data</p>
              )}
            </div>
            <Clock className={`w-8 h-8 ${isNewUser ? 'text-orange-600' : 'text-orange-600'}`} />
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm ${isNewUser ? 'text-orange-600' : 'text-purple-600'}`}>
                {isNewUser ? 'Demo Pending' : 'Pending Approval'}
              </p>
              <p className={`text-2xl font-bold ${isNewUser ? 'text-orange-700' : 'text-purple-700'}`}>
                {isNewUser ? 0 : state.runs.filter(run => run.status === 'awaiting_risk_approval' || run.status === 'awaiting_final_approval').length}
              </p>
              {isNewUser && (
                <p className="text-xs text-orange-600 mt-1">Mock data</p>
              )}
            </div>
            <AlertTriangle className={`w-8 h-8 ${isNewUser ? 'text-orange-600' : 'text-purple-600'}`} />
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-3">
            <div className="text-sm text-gray-600">
              {displayRuns.length} {isNewUser ? 'demo runs' : 'completed runs'}
            </div>
            {!isNewUser && (
              <button
        ...
        {displayRuns.length === 0 ? (
          <div className="text-center py-8 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-600 mb-2">
              {isNewUser ? 'No demo runs available' : 'No completed runs available'}
            </p>
            <p className="text-sm text-gray-500">
              {isNewUser 
                ? 'Demo data will appear here for testing purposes'
                : 'Complete some document reviews to see them here for replay'
              }
            </p>
          </div>
        ) : (
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  disabled={replaying || cloning || !selectedRun}
